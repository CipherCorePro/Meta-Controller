...
const LogPanel: React.FC<LogPanelProps> = ({ logs }) => {
    ...
    const getTranslatedSource = (source: LogEntry['source']) => {
        const key = `logs.source.${source}` as TranslationKey;
        const translated = t(key);
        // Fallback for keys that might not exist in a language file (e.g. 'rule-engine' vs 'ruleengine')
        return translated.startsWith('logs.source.') ? source.toUpperCase() : translated;
    };

    return (
        <div ref={scrollRef} className="bg-gray-900 p-3 rounded-md flex-grow overflow-y-auto font-mono text-xs h-full relative">
            <button
                onClick={handleExport}
                title={t('logs.export')}
                className="absolute top-2 right-2 p-1 bg-gray-700 hover:bg-cyan-500 rounded-full text-gray-300 hover:text-white transition-colors z-10"
            >
            ...
            <div className="relative">
                ...
                {logs.slice(0).reverse().map((log) => (
                    <div key={log.id} className="flex gap-2 items-start mb-1 animate-fade-in">
