...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, explanation, onCounterfactual }) => {
    ...
    // Simple counterfactual: suggest the next action in the list, wrapping around.
    const alternativeAction = agentState ? (agentState.lastAction + 1) % agentState.qValues.length : -1;
    const alternativeActionText = t(`q_action.${alternativeAction}` as TranslationKey);
                ...
                <div className="max-h-[80vh] overflow-y-auto">
                    ...
                    {!isLoading && content && agentState && (
                        <div className="p-4 border-t border-gray-700/50">
                            ...
                            {cfContent ? (
                                <div className="p-3 bg-gray-900/50 rounded-md text-gray-300 whitespace-pre-wrap leading-relaxed">{cfContent}</div>
                            ) : isCfLoading ? (
                                ...
                                >
                                    <span>{t('modal.counterfactual_button', { actionText: alternativeActionText })}</span>
                                </button>
                            )}
                        </div>
                    )}
