...
const ControlPanel: React.FC<ControlPanelProps> = ({ simulationState, onStateChange, onReset, onSave, onLoad }) => {
    ...
    return (
        <div className="bg-gray-900/70 backdrop-blur-sm border border-gray-700/50 p-4 rounded-lg shadow-lg flex flex-col sm:flex-row justify-between items-center gap-4 flex-wrap">
            <div className="flex items-center gap-2">
                ...
                >
                 ...
                 <button
                    onClick={onSave}
                    disabled
                    className="p-2 rounded-md bg-sky-500 text-white transition-all duration-200 opacity-50 cursor-not-allowed"
                    title="Save/Load is not applicable in this version"
                >
                    <ArrowDownTrayIcon className="w-5 h-5" />
                </button>
                 <button
                    onClick={onLoad as any}
                    disabled
                    className="p-2 rounded-md bg-sky-500 text-white transition-all duration-200 opacity-50 cursor-not-allowed"
                    title="Save/Load is not applicable in this version"
                >
