...
type LiveConfig = Pick<AgentConfig, 'learning_rate' | 'gamma' | 'epsilon' | 'frustration_threshold' | 'impulsive_exploration_boost' | 'enable_obstacles' | 'num_obstacles' | 'meta_cognitive_boost' | 'reflection_learning_rate'>;
...
const Slider: React.FC<{ label: string; value: number; min: number; max: number; step: number; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; disabled?: boolean; }> = ({ label, disabled = false, ...props }) => (
    <div className={disabled ? 'opacity-50' : ''}>
        <label className="flex justify-between items-center text-sm mb-1">
            ...
            <span className="font-mono text-cyan-300 bg-gray-700 px-2 py-0.5 rounded">{props.value.toFixed(props.step < 1 ? 2 : 0)}</span>
        </label>
        <input
            type="range"
            disabled={disabled}
...
const Toggle: React.FC<{ label: string; checked: boolean; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; }> = ({ label, checked, onChange }) => (
    <label className="flex items-center justify-between cursor-pointer">
        ...
        <div className="relative">
            ...
            <div className={`w-10 h-6 bg-gray-700 rounded-full peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-cyan-500 transition-colors ${checked ? 'bg-cyan-600' : ''}`}></div>
            <div className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${checked ? 'transform translate-x-full' : ''}`}></div>
        </div>
    </label>
);
        ...
        <div className="space-y-4">
            ...
            />
             <div className="border-t border-gray-700 pt-4 mt-4">
                 <Slider
                    label={t('config.reflection_learning_rate')}
                    value={liveConfig.reflection_learning_rate}
                    min={0} max={2} step={0.1}
                    onChange={handleChange('reflection_learning_rate')}
                />
            </div>
